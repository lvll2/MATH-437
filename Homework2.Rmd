...
rejections <- c(t.test(hwy ~ class, data = part_1_data)$p.value,t.test(hwy ~ class, data = part_2_data)$p.value,t.test(hwy ~ class, data = part_3_data)$p.value)
...
```{r}
bonferonni_values <- p.adjust(rejections, method = "bonferroni")
```
...
```{r}
holm_values <- p.adjust(rejections, method = "holm")
...
TukeyHSD(my_anova) # 95% confidence intervals
```
