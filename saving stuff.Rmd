...
sim2_values <- rbind(sim2_values,
  data.frame(condition = rep(c("Low", "High"), each = 10),
...
pvaluesB <- numeric(15000)
for(i in 1:length(pvaluesB)){
  Y_val <- rnorm(40, mean = 0, sd = 1)
  # Step 2: Randomly assign to low or high
  sim2_values <- data.frame(
    condition = rep(c("Low", "High"), each = 20),
...
# Step 4: Check if p-value is less than or equal to 0.05
  if (p_value1 <= .05 ){
    ...
    sim2_values <- rbind(sim2_values,
                         data.frame(condition = rep(c("Low", "High"), each = 10),
                                    Y1 = rnorm(20, mean = 0, sd = 1)))
    p_val = t.test(Y1 ~ condition, data = sim2_values)$p.value
  pvaluesB[i]= p_val
  }
  
}
mean(pvaluesB <= 0.05) # FPR at alpha = 5%
