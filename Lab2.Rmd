...
```{r power-default test-2, eval = FALSE}
Power_default(2)
```
...
Step 4: For i in 1 to number of permutation resamples:
  ...
  Step 4a: run a t-test on the permutation samples
  
  Step 4b: store the test statistics into the vector created in step 2
...
for (i in 1:10000){ # Complete the syntax
  ...
  # Step 4b
  t_perm[i] <- t.test(permutation_df$y ~permutation_df$group, alternative = "t")$statistic
}

# Step 5    
  # Use the course notes as a template for getting p_left and p_right
T_all <- c(t_obs, t_perm)
  p_left <- sum(T_all <= t_obs)/(t_perm+1)
  p_right <- sum(T_all >= t_obs)/(t_perm + 1)
