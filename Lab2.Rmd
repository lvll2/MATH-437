...
```{r}
alcohol_t_test$p_val
```
Since our p-value is `alcohol_t_test$p_val` , using a 5% significance level we can conclude that we should reject the null hypothesis that there is no difference between the groups which implies that our alternative hypothesis is correct and the SST program is effective at reducing alcohol intake.
