...
Since log$n$ > 2 for any $n > 7$, the BIC statistic generally places a heavier penalty on models with many variables than the AIC statistic.

5. In *ridge regression* and *LASSO*, what is the model when $\lambda = 0$? What about when $\lambda \rightarrow \infty$?

In ridge regression and lasso, when $\lambda = 0$ the model is the least squares estimates. As $\lambda \rightarrow \infty$, the ridge regression and lasso coefficient estimates will approach zero leaving us with the penalty term. 
