...
There are two purposes to this lab. First, you will get comfortable with the family-wise error rate and false discovery rate. Then, we will get more practice with coding a nonparametric bootstrap.
...
Run the code in ISLR Lab 13.6.1, with two exceptions. First, everywhere you see these three lines of code:

```{r decision DON'T RUN, eval = FALSE}
...
replace the code with the `tidyverse` equivalent by copying the chunk below (where appropriate) and completing the arguments to `if_else`.

```{r decision DO RUN, eval = FALSE}
decision_df <- tibble(p_value = p.values,
                          truth = c(rep("H0 is False", 50), rep("H0 is True", 50))
) %>% mutate(
  decision = if_else()
)
xtabs( ~ decision + truth, data = decision_df) # if we have a count variable it goes on the left side of the ~
```

Second, instead of creating an entirely new `x` in the last chunk, simply add 0.5 to the first 50 rows of the already-existing `x` again.

## Part b (Code: 1 pt)

Run the code in ISLR Lab 13.6.2. Put each chunk from the textbook in its own chunk.
...
Compare your results from the three procedures.
...
Hint: Use the Jackknife and Bootstrap class activity as a template, but resample the *row numbers* (i.e. 1:n) to pass in as the `index` argument in `alpha.fn` rather than resampling a from a vector of data.
...
Run the code in the second half of ISLR Lab 5.3.4, "Estimating the Accuracy of a Linear Regression Model." Put each chunk from the textbook in its own chunk.

## Part b (Code: 1 pt)

According to the lab, "there is a non-linear relationship in the data, and so the residuals from a linear fit will be inflated, and so will $\hat{\sigma}^2$." To verify the nonlinear relationship, reproduce the scatterplot, orange line, and blue curve in Figure 3.8 from the textbook. 

Hint 1: Call the `coef()` function to obtain the coefficients of the model, as shown in the very last chunk of Part a. This will make fitting the lines much easier.

Hint 2: Use the `seq()` function to create a sequence of evenly spaced horsepower values. If you created your scatterplot with Base R, you can add the orange and blue lines using the `lines()` function. If you are plotting using the `ggplot2` package, create a data frame with the horsepower values, the $\hat{mpg}$ values from the linear model, and the $\hat{mpg}$ values from the quadratic model, then add two `geom_line` objects to your scatterplot.

## Part c (Code: 1 pt)

Similarly to Problem 4b, write your own code chunk that fits the quadratic model on a bootstrap sample from the `Auto` dataset, records all the coefficient estimates, and computes the resulting standard deviation of all three coefficient estimates.

Hint: In Problem 5a you already defined `boot.fn` to return the coefficient vector. Store that vector you get from `boot.fn` in the $i^th$ row of `boot_estimates` defined in the chunk below; then as shown in the chunk, use the `apply` function to get all three standard errors with one line of code.

```{r Problem 5c standard error, eval = FALSE}
boot_estimates <- matrix(0, nrow = 1000, ncol = 3)
colnames(boot_estimates) <- c("B0", "B1", "B2")
# 1000 bootstrap estimates of B0, B1, B2
...
# computes the standard deviation of each column of boot_estimates
...
Use a quick-and-dirty confidence interval method ($\text{statistic} \pm 2 \times SE_{boot}$) to obtain an estimated 95% confidence interval for each parameter in the model.
