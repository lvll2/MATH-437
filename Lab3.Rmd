...
reject.falseH0 <- sum(p.adjust(p_values2[1:25],  method = "holm") <= alpha)
reject.trueH0 <- sum(p.adjust(p_values2[26:100],  method = "holm") <= alpha)
c(true = reject.trueH0, false = reject.falseH0)
```
...
```{r 13.7.8d bh redux}
reject.falseH0 <- sum(p.adjust(p_values2[1:25],  method = "BH") <= alpha)
reject.trueH0 <- sum(p.adjust(p_values2[26:100],  method = "BH") <= alpha)
c(true = reject.trueH0, false = reject.falseH0)
