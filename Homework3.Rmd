...
$\beta_3$ is the estimate for how much we expect sales to be affected for every $1,000 increase in spending on newspaper advertising, if we hold TV and radio advertising constant.
...
To turn a factor variable with k classes into a dummy variables we must choose a baseline class which we will assign the value 0 to and create new variables for the remaining k - 1 classes that are assigned the value 1 (or we can use -1 and 1 rather than 0 and 1).
